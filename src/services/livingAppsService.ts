...
export class LivingAppsService {
  ...
  // --- MEINE_BESTELLUNG ---
  static async getMeineBestellung(): Promise<MeineBestellung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEINE_BESTELLUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as MeineBestellung[];
    return enrichLookupFields(records, 'meine_bestellung');
  }
  static async getMeineBestellungEntry(id: string): Promise<MeineBestellung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEINE_BESTELLUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as MeineBestellung;
    return enrichLookupFields([record], 'meine_bestellung')[0];
  }
  static async createMeineBestellungEntry(fields: CreateMeineBestellung) {
    return callApi('POST', `/apps/${APP_IDS.MEINE_BESTELLUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'meine_bestellung') });
  }
  static async updateMeineBestellungEntry(id: string, fields: Partial<CreateMeineBestellung>) {
    return callApi('PATCH', `/apps/${APP_IDS.MEINE_BESTELLUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'meine_bestellung') });
  }
